...
void SysfsPollingOneShotSensor::writeEnable(bool enable) {
    int fd = open(mEnablePath.c_str(), O_WRONLY | O_CLOEXEC);
    if (fd >= 0) {
        char c = enable ? '1' : '0';
        ssize_t written = write(fd, &c, 1);
        if (written < 0) {
            ALOGE("Failed to write enable to %s: %s", mEnablePath.c_str(), strerror(errno));
        }
        close(fd);
    } else {
        ALOGE("Failed to open %s for write: %s", mEnablePath.c_str(), strerror(errno));
    }
}
